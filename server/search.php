...
$result = $conn->query("SELECT `id`, `creatorid`, `tag`, `detail`, `created`, `price` FROM `products` WHERE 1");
    ...
    <div class="product-card-container">
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $tag = htmlspecialchars($row['tag']);  // Tag (escaped for XSS)
                $description = htmlspecialchars($row['detail']);  // Description (escaped for XSS)
                $created = $row['created'];  // Date created
                $price = number_format($row['price'], 2);  // Dynamic price (formatted)
